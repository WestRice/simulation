...
//--------------------------------------------------------------------------------------
void RenderText(float elapsedTime)
{
...
//    g_pTxtHelper->DrawTextLine( DXUTGetDeviceStats() );
	g_pTxtHelper->DrawTextLine((L"t="+os.str()).c_str());
    g_pTxtHelper->End();
}
                                      ...
                                      void* pUserContext )
{
	...
	DXUTCreateSphere(pd3dDevice, 1.0f, 32, 16, &g_Mesh);
    // Load the texture
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"misc\\galaxy.dds" ) );
    V_RETURN( CreateDDSTextureFromFile( pd3dDevice, str, &g_pSRV10 ) );

    // Setup the camera's view parameters
    D3DXVECTOR3 vecEye( 0.0f, 50.0f, 0.0f );
...
float expand(float elapsedTime, float a)
{
	int n = static_cast<int>(elapsedTime / 0.01f);

	float H = 0.0f;
	float da;
	for (int i = 0; i < n; ++i)
	{
		H = 0.678f*sqrt(0.315f/(a*a*a) + (9.24e-5)/(a*a) + 0.6817f);
		da = a*(exp(H*0.01f) - 1);
		a = a + da;
	}
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (a < 25.0f)
	{
		a = expand(fElapsedTime, a);
		D3DXMatrixScaling(&scale, a, a, a);
		mWorld *= scale;
	}
	...
	g_pRenderScene->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		g_pRenderScene->GetPassByIndex(p)->Apply(0);
